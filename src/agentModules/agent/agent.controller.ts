...
import {
  ...
  CreateAgentInputDto,
  UpdateAgentDto,
  toCreateAgentDto, // <-- use helper to preserve provider/model fields
} from './dto/agent.dto';
...
class AllModelOptionsDto {
  ...
  @ApiProperty({
    ...
    example: {
      CHATGPT: [{ value: 'gpt_4', label: 'Gpt 4' }],
      GEMINI: [{ value: 'gemini_1_5_flash', label: 'Gemini 1 5 Flash' }],
      CLAUDE: [{ value: 'claude_3_5_sonnet', label: 'Claude 3 5 Sonnet' }],
    },
  })
...
export class AgentController {
  ...
  private toAgentResponseDto(a: Agent): AgentResponseDto {
    ...
    return {
      ...
      isKnowledgebaseActive: (a as any).isKnowledgebaseActive,
      isBookingActive: (a as any).isBookingActive,
      modelType: a.modelType as AIModel,
      useOwnApiKey: (a as any).useOwnApiKey,
  ...
  async create(
    ...
    // Use the DTO helper to carry provider/model fields and alias apiKey properly
    ...
    return {
      ...
      links: this.buildResourceLinks(req, agent.id),
      meta: this.buildHttpMeta(req, HttpStatus.CREATED, 'Created'),
    };
  }
  ...
  async findAllMine(
    ...
    return {
      ...
      meta: paginationMeta,
      links: this.buildPaginationLinks(req, paginationMeta),
      http: this.buildHttpMeta(req, HttpStatus.OK, 'OK'),
    };
  }
  ...
  async findOne(
    ...
    return {
      data: this.toAgentResponseDto(agent),
      links: this.buildResourceLinks(req, agent.id),
      meta: this.buildHttpMeta(req, HttpStatus.OK, 'OK'),
    };
  }
